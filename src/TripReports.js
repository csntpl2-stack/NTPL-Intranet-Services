...
const TripReports = () => {
  ...
  useEffect(() => {
    ...
    try {
      const files = importAll(require.context('./tripreports', true, /\.(pdf|docx|xlsx)$/));
      ...
      <div className="container mt-5">
        <div className="card shadow-lg">
          <div className="card-header bg-primary text-white">
            <h3 className="text-center">Trip Reports</h3>
          </div>
          ...
          <div className="card-footer text-muted text-center">
            Total Years: {Object.keys(reportFiles).length}
          </div>
        </div>
      </div>
    </div>
